...
> Авторизация **замокана** в `src/data/mockAccounts.ts`. После готовности бэкенда замените `login` / `register` в `AuthContext` на JWT.
...
Регистрация (`/register`) сохраняет пользователя в памяти сессии до перезагрузки страницы.
...
1. Подключить JWT: `POST /api/auth/login`, `POST /api/auth/register`, хранение токена, замена моков в `AuthContext`.
